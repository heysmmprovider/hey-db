...
const theme = EditorView.theme({ '&': { height: '100%', backgroundColor: 'transparent', color: 'var(--text)' }, '.cm-scroller': { overflow: 'auto', fontFamily: 'var(--mono)', fontSize: '13px', lineHeight: '1.9' }, '.cm-content': { padding: '18px 0' }, '.cm-gutters': { background: 'transparent', border: 'none', color: 'var(--faint)' }, '.cm-lineNumbers .cm-gutterElement': { padding: '0 20px 0 18px' }, '.cm-activeLineGutter': { backgroundColor: 'transparent', color: 'var(--text)' }, '.cm-activeLine': { background: 'var(--editor-line)' }, '&.cm-focused': { outline: 'none' }, '.cm-content ::selection, .cm-content::selection': { background: 'var(--selection) !important' }, '.cm-cursor': { borderLeftColor: 'var(--accent)' }, '.cm-tooltip': { background: 'var(--panel)', color: 'var(--text)', border: '1px solid var(--line)', borderRadius: '6px' } });
const SqlEditor = forwardRef<EditorHandle, { value: string; onChange: (value: string) => void; onRun: () => void; schema: SQLNamespace }>(({ value, onChange, onRun, schema }, ref) => {
  ...
  useEffect(() => {
    ...
    const instance = new EditorView({ parent: host.current, state: EditorState.create({ doc: value, extensions: [lineNumbers(), history(), highlightActiveLine(), highlightActiveLineGutter(), bracketMatching(), closeBrackets(), autocompletion(), language.current.of(sql({ dialect: PostgreSQL, schema })), syntaxHighlighting(syntax), theme, EditorView.contentAttributes.of({ 'aria-label': 'SQL editor', spellcheck: 'false', autocorrect: 'off', autocapitalize: 'none' }), keymap.of([{ key: 'Mod-Enter', run: () => { callbacks.current.onRun(); return true; } }, ...completionKeymap, ...closeBracketsKeymap, ...defaultKeymap, ...historyKeymap, indentWithTab]), EditorView.updateListener.of(update => { if (update.docChanged) callbacks.current.onChange(update.state.doc.toString()); })] }) });

  ...
  return <Dialog title={existing ? 'Connect to PostgreSQL' : 'New connection'} onClose={onClose} busy={busy}>
    ...
    <form onSubmit={async e => { e.preventDefault(); setBusy(true); setError(''); try { await onConnect({ ...profile, name: profile.name.trim() || profile.database }, existing?.rememberPassword && !touched ? undefined : password); onClose(); } catch (err) { setError(message(err)); } finally { setBusy(false); } }}>
      <fieldset disabled={busy}>
        <label>Connection name<input autoCorrect="off" autoCapitalize="none" spellCheck={false} autoFocus placeholder="e.g. Local development" value={profile.name} onChange={e => field('name', e.target.value)} /></label>
        <div className="form-row host-row"><label>Host<input autoCorrect="off" autoCapitalize="none" spellCheck={false} required value={profile.host} onChange={e => field('host', e.target.value)} /></label><label>Port<input autoCorrect="off" autoCapitalize="none" spellCheck={false} required type="number" min="1" max="65535" value={profile.port} onChange={e => field('port', Number(e.target.value))} /></label></div>
        <label>Database<input autoCorrect="off" autoCapitalize="none" spellCheck={false} required value={profile.database} onChange={e => field('database', e.target.value)} /></label>
        <div className="form-row"><label>Username<input autoCorrect="off" autoCapitalize="none" spellCheck={false} required value={profile.username} onChange={e => field('username', e.target.value)} autoComplete="off" /></label><label>Password<input autoCorrect="off" autoCapitalize="none" spellCheck={false} type="password" value={password} placeholder={existing?.rememberPassword ? 'Use saved password' : 'Password'} onChange={e => { setTouched(true); setPassword(e.target.value); }} autoComplete="off" /></label></div>
        ...
        <label className="check-label"><input autoCorrect="off" autoCapitalize="none" spellCheck={false} type="checkbox" checked={profile.rememberPassword} onChange={e => field('rememberPassword', e.target.checked)} />Remember password in system credential storage</label>
        <label className="check-label"><input autoCorrect="off" autoCapitalize="none" spellCheck={false} type="checkbox" checked={profile.readOnly} onChange={e => field('readOnly', e.target.checked)} />Read-only connection</label>
      </fieldset>
...
export default function App() {
  ...
  const [tableCache, setTableCache] = useState<Record<string, TableInfo[]>>({});
  const tables = useMemo(() => active ? tableCache[active.id] ?? [] : [], [active, tableCache]);
  ...
  const [connectionDialog, setConnectionDialog] = useState<{ existing?: Profile } | null>(null);
  ...
  const [exportFormat, setExportFormat] = useState<ExportFormat>(() => { const saved = localStorage.getItem('heydb.exportFormat'); return saved === 'json' || saved === 'tsv' ? saved : 'csv'; });
  const [exportMenu, setExportMenu] = useState(false);
  const [exportSettings, setExportSettings] = useState(false);
  const exportControls = useRef<HTMLDivElement>(null);
  useEffect(() => { localStorage.setItem('heydb.exportFormat', exportFormat); }, [exportFormat]);
  useEffect(() => {
    const close = (e: PointerEvent) => { if (!exportControls.current?.contains(e.target as Node)) { setExportMenu(false); setExportSettings(false); } };
    const escape = (e: KeyboardEvent) => { if (e.key === 'Escape') { setExportMenu(false); setExportSettings(false); } };
  ...
  const ensureConnected = useCallback(async (profile: Profile) => {
    ...
    try { await api.connect(profile); }
      ...
      if (/password|authentication|credential/i.test(message(err))) setConnectionDialog({ existing: profile });
      throw err;
    }
    sessions.current.add(profile.id); setConnected(new Set(sessions.current));
  }, []);
  const loadTables = async (profile: Profile) => {
    if (inFlight.current) return;
    inFlight.current = true; setRefreshing(true); setError('');
    try { await ensureConnected(profile); const loaded = await api.tables(profile.id); setTableCache(cache => ({ ...cache, [profile.id]: loaded })); }
    catch (err) { setError(message(err)); }
    finally { inFlight.current = false; setRefreshing(false); }
  };
  const selectDatabase = (profile: Profile) => {
    if (active?.id !== profile.id) { setActive(profile); resetResult(); }
    setExpanded(current => new Set(current).add(profile.id));
  };
  const run = useCallback(async (sqlOverride?: string, profileOverride?: Profile, ignoreEdits = false, table?: TableInfo) => {
  ...
  const connect = async (profile: Profile, password?: string) => {
    ...
    try { await api.saveProfile(profile, password); } catch (err) { await api.disconnect(profile.id); sessions.current.delete(profile.id); setConnected(new Set(sessions.current)); throw err; }
    sessions.current.add(profile.id); setConnected(new Set(sessions.current));
    setProfiles(await api.profiles()); setActive(profile); resetResult(); setExpanded(current => new Set(current).add(profile.id)); await loadTables(profile); setNotice(`Connected to ${profile.name}`);
  };
  const enterDemo = async () => { selectDatabase(demoProfile); setTabs([{ id: 'demo-query', name: 'Active products.sql', sql: DEFAULT_SQL }]); setTabId('demo-query'); await loadTables(demoProfile); await run(DEFAULT_SQL, demoProfile, true); };
  const browse = (table: TableInfo, profile: Profile) => guard(() => {
    selectDatabase(profile);
    ...
    void run(query, profile, true, table);
  });
  const stage = useCallback((edit: CellEdit) => { if (result && !applying) { setEdits(current => stageEdit(current, result, edit)); setNotice(''); } }, [result, applying]);
  const preview = async () => {
  ...
  const exportResults = async (destination: 'file' | 'clipboard') => {
    if (!result) return;
    setExportMenu(false);
    try {
      const data = exportData(result, exportFormat);
      if (destination === 'clipboard') { await navigator.clipboard.writeText(data.text); }
      else if (desktop) {
        const { save } = await import('@tauri-apps/plugin-dialog'); const { writeTextFile } = await import('@tauri-apps/plugin-fs');
        const path = await save({ defaultPath: `query-results.${exportFormat}`, filters: [{ name: exportFormat.toUpperCase(), extensions: [exportFormat] }] });
        if (!path) return;
        await writeTextFile(path, data.text);
      } else {
        const url = URL.createObjectURL(new Blob([data.text], { type: data.mime })); const link = document.createElement('a');
        link.href = url; link.download = `query-results.${exportFormat}`; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000);
      }
      setNotice(`${destination === 'clipboard' ? 'Copied' : 'Saved'} loaded results as ${exportFormat.toUpperCase()}. Pending edits are excluded.`);
    } catch (err) { setError(message(err)); }
  };
  ...
  return <div className={`app ${sidebar ? '' : 'sidebar-hidden'} ${!desktop ? 'browser-preview' : ''}`}>
    ...
    <div className="workspace">
      {sidebar && <aside className="sidebar">
        ...
        <div className="sidebar-search"><Search size={14} /><input autoCorrect="off" autoCapitalize="none" spellCheck={false} aria-label="Filter tables" placeholder="Find a table…" value={filter} onChange={e => setFilter(e.target.value)} /></div>
        <div className="explorer-heading"><span><ChevronDown size={13} /><Database size={13} />Databases</span></div>
        <nav className="database-tree" aria-label="Databases">
          {[...profiles, ...(tableCache[DEMO_ID] || active?.id === DEMO_ID ? [demoProfile] : [])].map(profile => {
            const loaded = tableCache[profile.id]; const open = expanded.has(profile.id);
            const schemas = [...new Set((loaded ?? []).map(table => table.schema))];
            return <div key={profile.id}>
              ...
              {open && <div className="database-schemas">{schemas.map(schema => {
                const key = `${profile.id}:${schema}`;
                return <div key={schema}><button className="schema-heading" aria-expanded={!collapsed.has(key)} onClick={() => setCollapsed(current => { const next = new Set(current); next.has(key) ? next.delete(key) : next.add(key); return next; })}>{collapsed.has(key) ? <ChevronRight size={13} /> : <ChevronDown size={13} />}<span>{schema}</span><small>{loaded!.filter(t => t.schema === schema).length}</small></button>{!collapsed.has(key) && loaded!.filter(t => t.schema === schema && `${t.schema}.${t.name}`.toLowerCase().includes(filter.toLowerCase())).map(table => <button className={`table-item ${active?.id === profile.id && selectedTable?.oid === table.oid ? 'active' : ''}`} key={table.oid} disabled={busy || applying || refreshing} onClick={() => browse(table, profile)} title={`${table.schema}.${table.name}`}><Table2 size={14} /><span>{table.name}</span>{['v', 'm'].includes(table.kind) && <small>view</small>}</button>)}</div>;
              })}{!loaded && <p className="database-hint">Run a query to connect, or refresh to load tables.</p>}{loaded?.length === 0 && <p className="database-hint">No user tables found.</p>}</div>}
            </div>;
          })}
      ...
      <main className="main">
        ...
        {!active ? <div className="welcome"><div className="welcome-mark"><Database size={33} strokeWidth={1.5} /></div><div className="eyebrow">A FOCUSED DATABASE WORKSPACE</div><h1>Make yourself at home<br />in your database.</h1><p>Explore your tables, write SQL, and make thoughtful changes.<br />Everything stays on your computer.</p><div className="welcome-actions"><button className="primary-button" onClick={() => setConnectionDialog({})}><Plus size={16} />Connect to PostgreSQL</button><button className="secondary-button" onClick={() => void enterDemo()}>Explore the demo<ArrowRight size={15} /></button></div><div className="welcome-footnote"><ShieldCheck size={14} />No account. No telemetry. Just your workspace.</div></div> : <>
          ...
          <section className="results-pane" aria-label="Results workspace"><div className="results-toolbar"><div className="result-tabs"><button className={view === 'results' ? 'active' : ''} onClick={() => setView('results')}><Table2 size={14} />Results{result && <span className="count">{result.rows.length.toLocaleString()}</span>}</button><button className={view === 'structure' ? 'active' : ''} disabled={!details} onClick={() => setView('structure')}>Structure</button></div><div className="result-actions">{cell && result?.columns[cell.column].editable && <button className="text-button" disabled={busy || applying} onClick={() => stage({ ...cell, value: null })}>Set NULL</button>}<button className="icon-button" aria-label="Refresh results" title="Refresh results" disabled={!result || busy || applying || edits.length > 0} onClick={() => void run(lastSql.current)}><RefreshCw size={14} /></button><div className="export-controls" ref={exportControls}>
                ...
                {exportSettings && <div className="export-popover"><label>Data format<select value={exportFormat} onChange={e => setExportFormat(e.target.value as ExportFormat)}><option value="csv">CSV</option><option value="json">JSON</option><option value="tsv">TSV</option></select></label></div>}
              </div></div></div>
    ...
    <footer className="statusbar"><span><i className={`status-dot ${!active || (active.id !== DEMO_ID && !connected.has(active.id)) ? 'offline' : ''}`} />{active ? active.id === DEMO_ID ? 'Demo workspace · No database connected' : `${active.name} · ${connected.has(active.id) ? 'Connected' : 'Connects on demand'}` : 'Ready when you are'}{active?.readOnly && <LockKeyhole size={11} />}</span><span>{active && <><span>PostgreSQL</span><i className="status-divider" /></>}UTF-8<span className="status-divider" />Local first</span></footer>
